...
abstract class zzefz<T extends zzefo> {
    ...
    static <T extends zzefo> T zzc(Class<T> cls) {
        ...
        try {
            return (zzefo) cls.cast(((zzefz) Class.forName(str, true, classLoader).getConstructor(new Class[0]).newInstance(new Object[0])).zzbfb());
            ...
            while (it.hasNext()) {
                try {
                    arrayList.add(cls.cast(((zzefz) it.next()).zzbfb()));
